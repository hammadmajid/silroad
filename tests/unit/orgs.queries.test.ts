...
describe('OrganizationRepo - Query Methods', () => {
	...
	describe('getOrganizationWithEvents', () => {
		it('should return organization with its events', async () => {
			const mockOrgWithEvents = {
				...mockOrg,
				events: [
					{ id: 'event-1', title: 'Test Event 1', dateOfEvent: new Date('2024-12-01') },
					{ id: 'event-2', title: 'Test Event 2', dateOfEvent: new Date('2024-12-15') }
				]
			};
			...
			mockDb.where.mockReturnValue(mockDb);
			mockDb.groupBy.mockResolvedValue([mockOrgWithEvents]);

			const result = await orgRepo.getOrganizationWithEvents('org-1');

			expect(result).toEqual(mockOrgWithEvents);
			expect(mockDb.leftJoin).toHaveBeenCalled();
			expect(mockDb.groupBy).toHaveBeenCalled();
		});

		it('should return organization with empty events array when no events', async () => {
			const mockOrgWithEvents = {
				...mockOrg,
				events: []
			};
		...
		it('should return null when organization not found', async () => {
			...
			const result = await orgRepo.getOrganizationWithEvents('nonexistent');

			expect(result).toBeNull();
		});

		it('should validate UUID format for organizationId', async () => {
			const result = await orgRepo.getOrganizationWithEvents('invalid-uuid');
		...
		it('should order events by date', async () => {
			const mockOrgWithEvents = {
				...mockOrg,
				events: [
					{ id: 'event-1', title: 'Future Event', dateOfEvent: new Date('2024-12-01') },
					{ id: 'event-2', title: 'Near Future Event', dateOfEvent: new Date('2024-11-15') }
				]
			};
			...
			mockDb.orderBy = vi.fn().mockResolvedValue([mockOrgWithEvents]);
		...
		it('should include event details like title, description, and date', async () => {
			const mockOrgWithEvents = {
				...mockOrg,
				events: [
					{
